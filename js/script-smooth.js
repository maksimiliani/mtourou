...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  document.querySelectorAll('.container > div').forEach(item => {
    item.addEventListener("mousemove", handleMouseOver);
...
function expandCell(target, smooth = false, double_click = true, scale = "15") {
  if ((cell.lastTriggered == target.target) && !double_click && !smooth) return;
  ...
  for (let i = 1; i <= maxcount.col; i++) {
    if (smooth && (i + 1 == inx.h )) {
      newmap.col += (scale*element_pos_p.h + (maxcount.col - Math.abs(inx.h - i))/(maxcount.col)*(1 - element_pos_p.h) + 'fr ');
    } else if (smooth && (i - 1 == inx.h )) {
      newmap.col += (scale*(1 - element_pos_p.h) + (maxcount.col - Math.abs(inx.h - i))/(maxcount.col)*element_pos_p.h + 'fr ');
    } else {
  ...
  for (let i = 1; i <= maxcount.row; i++) {
    if (smooth && (i + 1 == inx.v )) {
      newmap.row += (scale*element_pos_p.v + (maxcount.row - Math.abs(inx.v - i))/(maxcount.row)*(1 - element_pos_p.v) + 'fr ');
    } else if (smooth && (i - 1 == inx.v )) {
      newmap.row += (scale*(1 - element_pos_p.v) + (maxcount.row - Math.abs(inx.v - i))/(maxcount.row)*element_pos_p.v + 'fr ');
    } else {
...
function deductWithValueEase(startValue, endValue, duration, currentTime) {
  ...
  return {x: startValue.x + (endValue.x - startValue.x) * easedProgress, y: startValue.y + (endValue.y - startValue.y) * easedProgress};
    ...
    function handleMouseOver(e) {
      ...
      element_pos_p = {
        h: Math.abs((cursorX - e.target.getBoundingClientRect().x)/e.target.getBoundingClientRect().width),
        v: Math.abs((cursorY - e.target.getBoundingClientRect().y)/e.target.getBoundingClientRect().height),
      }
      console.log(element_pos_p);
